...
{
    public class QuartOutIn : EaseBase
    {
        private static readonly QuartOut QuartOut = new QuartOut();
        private static readonly QuartIn QuartIn = new QuartIn();
        public override Color GetEasedValue(Color start, Color end, float step)
        {
            step = Mathf.Clamp01(step);
            if (step < .5f)
                return QuartOut.GetEasedValue(start, start + (end - start) / 2, step * 2);
            return QuartIn.GetEasedValue(start + (end - start) / 2, end, step * 2 - 1);
        }
