...
{
    public class SineOutIn:EaseBase
    {
        private static readonly SineIn SineIn = new SineIn();
        private static readonly SineOut SineOut = new SineOut();
        public override Color GetEasedValue(Color start, Color end, float step)
        {
            step = Mathf.Clamp01(step);
            if (step < .5f)
                return SineOut.GetEasedValue(start, start + (end - start) / 2, step * 2);
            return SineIn.GetEasedValue(start + (end - start) / 2, end, step * 2 - 1);
        }
