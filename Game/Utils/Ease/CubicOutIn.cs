...
{
    public class CubicOutIn : EaseBase
    {
        private static readonly CubicIn CubicIn = new CubicIn();
        private static readonly CubicOut CubicOut = new CubicOut();
        public override Color GetEasedValue(Color start, Color end, float step)
        {
            step = Mathf.Clamp01(step);
            if (step < .5f)
                return CubicOut.GetEasedValue(start, start + (end - start) / 2, step * 2);
            return CubicIn.GetEasedValue(start + (end - start) / 2, end, step * 2 - 1);
        }
